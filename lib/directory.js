...
Directory.prototype.process = function() {
  ...
  .otherwise(function(er) {
    console.log('Directory'.yellow, 'not found'.red);
    ...
    //delete opts.parent.children[self.path];
...
Directory.prototype.iterate = function(fileName){
  ...
  if ( self.exclude(fileName, path) ) return;
  ...
  options = _.extend( _.clone(self.options), {
    ...
    parent: self,
    output: self.output,
    destination: (self.destination && join(self.destination, fileName)),
    // All instances share children ['full/path/to/file.js'] unless object format
    //children: !self.asObject && self.children,
...
Directory.prototype.exclude = function(fileName, path) {
  ...
  return (self.white_list && !_.include(self.white_list, fileName)) ||
...
Directory.prototype._watch = function(){
  ...
  var self = this;
  if (self.watch == false || self.watch == 'files' || self.watcher)
    return 
  ...
  //@file_watchers.push \
  self.watcher = fs.watch(self.path, _.bind(self.watchHandler, self))
