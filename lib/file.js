var
  ...
  debug        = require('debug')('loaddir:file'),
  File
  ;
    ...
    if (self.require) {
      ...
      .then(function(){
        try {
          ...
          debug('error compiling'.red, self.path.blue, er);
        }
        try {
          if (self.callback)
            self.fileContents = self.callback(self);
        } catch (er) {
          debug('error calling back'.red, self.path.blue, er);
        }
      });
...
File.prototype.read = function() {
  ...
  debug('File'.magenta, 'read'.blue, self.path.magenta);
...
File.prototype._watchHandler = function() {
  ...
  var self = this;
  debug('File'.magenta, 'watchHandler'.blue, this.path.green);
