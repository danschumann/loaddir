...
File.prototype.process = function() {
    ...
    if (self.require) {
      ...
      .then(function(){
        try {
          ...
          console.log('error compiling'.red, self.path.blue, er);
        }
        try {
          if (self.callback)
            self.fileContents = self.callback(self);
        } catch (er) {
          console.log('error calling back'.red, self.path.blue, er);
        }
      });
  ...
  .otherwise(function(er) {
    console.log('Error loading file -- Perhaps it was deleted'.red, self.path.blue, er);
